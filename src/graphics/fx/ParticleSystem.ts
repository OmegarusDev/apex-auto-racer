...
import { writeWorldToScreen } from '../coords';
...
export class ParticleSystem {
  /** @deprecated alias */
  static Particles = ParticleSystem;
  ...
  renderGround(
    ...
    for (let i = 0; i < SKID_CAP; i++) {
      ...
      const a = t * 0.55;
      writeWorldToScreen(s.x1, s.y1, camera, screenW, screenH, scratchA);
      writeWorldToScreen(s.x2, s.y2, camera, screenW, screenH, scratchB);
      ctx.strokeStyle = `rgba(20,20,24,${a})`;
      ctx.lineWidth = 2.5 * camera.zoom;
      ctx.beginPath();
      ctx.moveTo(scratchA.x, scratchA.y);
      ctx.lineTo(scratchB.x, scratchB.y);
      ctx.stroke();
    }
    ...
    for (let i = 0; i < DUST_CAP; i++) {
      ...
      writeWorldToScreen(d.x, d.y, camera, screenW, screenH, scratchA);
      ctx.fillStyle = `rgba(180,160,130,${t * 0.35})`;
      ctx.beginPath();
      ctx.arc(scratchA.x, scratchA.y, d.size * camera.zoom * (1 + (1 - t) * 0.5), 0, Math.PI * 2);
      ctx.fill();
    }
  ...
  renderAir(
    ...
    for (let i = 0; i < SMOKE_CAP; i++) {
      ...
      if (!s || s.life <= 0) continue;
      const t = s.life / s.maxLife;
      writeWorldToScreen(s.x, s.y, camera, screenW, screenH, scratchA);
      ctx.fillStyle = `rgba(80,80,88,${t * 0.25})`;
      ctx.beginPath();
      ctx.arc(scratchA.x, scratchA.y, s.size * camera.zoom * (1 + (1 - t)), 0, Math.PI * 2);
      ctx.fill();
    }
    ...
    for (let i = 0; i < SPARK_CAP; i++) {
      ...
      const t = s.life / s.maxLife;
      writeWorldToScreen(s.x, s.y, camera, screenW, screenH, scratchA);
      writeWorldToScreen(s.x - s.vx * 0.04, s.y - s.vy * 0.04, camera, screenW, screenH, scratchB);
  ...
  private tickRain(dt: number): void {
    ...
    while (this.rainSpawnAcc >= 1) {
      ...
      const idx = i;
      let slot = this.rain[i];
      const life = 0.8 + hashUnit(idx, 79) * 0.4;
      if (slot === undefined) {
        slot = {
          x: 0,
          y: 0,
      ...
      slot.x = hashUnit(idx, 61) * this.screenW;
      slot.y = -hashUnit(idx, 67) * this.screenH * 0.25;
      slot.vy = 420 + hashUnit(idx, 71) * 180;
      slot.len = 12 + hashUnit(idx, 73) * 16;
      slot.life = life;
      slot.maxLife = life;
