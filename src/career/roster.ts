...
import type { DisciplineId, Driver, GameState } from '../engine/types';
...
export function generateFreeAgents(state: GameState, discipline: DisciplineId, rerollOffset = 0): Driver[] {
  ...
  for (let i = 0; i < BALANCE.freeAgentPoolSize; i++) {
    agents.push(generateDriver(rng, baseMin * 4, baseMax * 4, used, discipline));
  }
  return agents;
}
