...
import { getDisciplineProfile, profileDeslotMinTime } from '../../data/disciplineProfiles';
...
export function updateVehicle(
  ...
  if (mode === 'deslot') {
    ...
    const scrubGain = PHYSICS.deslotScrubGain * getDisciplineProfile(ctx.discipline).deslotScrubMult;
  ...
  if (Math.abs(car.l) > wallLimit) {
    ...
    if (hard) {
      ...
      const streetStun = getDisciplineProfile(ctx.discipline).wallStunMult;
